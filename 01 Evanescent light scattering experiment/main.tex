...
The linear fit for the free particles, works as described with a $D_0 = \SI{1.013(29)}{\micro m^2/s}$.
This is equivalent with the expected value of $D_0 = \frac{k_B T}{6 \pi \eta r} = \SI{1.22(18)}{\micro m ^2 / s}$ \cite{instructions}, with $r = \SI{1.86(27)}{\micro m}$ and $\eta = \SI{0.955}{mPa\cdot s}$\cite{n:water}.

For a confined particle the MSD reaches a plateau at $\text{MSD}(\infty)$ \cite{instructions}.
For the higher spring stiffnesses (\SI{0.90}{}, \SI{1.01}{}), this happens and the spring stiffness are shown in \autoref{fig:01spring}.\\
